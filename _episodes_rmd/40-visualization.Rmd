...
> The idea behind the Grammar of Graphics is that you can build every
graph from the same three components: (1) a data set, (2) a coordinate system,
...
log2 transformation to have a more symmetric distribution. Note that, here, we
add a small constant value (`+1`) to avoid having `-Inf` values
...
Or to colour each gene in the plot differently, you could use a vector as
...
change the look of your visualisation. The complete list of themes is
...
data* and *tidy tools* pipeline. There are other visualisation packages
in R that should not be ignored.
